(() => {
    var store = localStorage; // keep an instance of localStorage before Discord steals it.

    window.GLOBAL_ENV = {
        API_ENDPOINT: '//discrod.bin.moe/api',
        WEBAPP_ENDPOINT: '//discrod.bin.moe/',
        CDN_HOST: 'discrod.bin.moe',
        ASSET_ENDPOINT: '//discrod.bin.moe/',
        WIDGET_ENDPOINT: '//discrod.bin.moe/widget',
        INVITE_HOST: 'i.bin.moe',
        MARKETING_ENDPOINT: '//discrod.bin.moe',
        NETWORKING_ENDPOINT: '//discrod.bin.moe',
        RELEASE_CHANNEL: 'stable',
        SUPPORT_URL: '//discrod.bin.moe/invite/anime',
    };
    ...
    function log(message) {
        console.log("%c[discrod] ", "color:orange", message);
    }
    ...
    window.discrod_token = token => {
        if (typeof token === "string") {
            log("Injected a new token, reload to see changes!");
        } else {
    ...
    function loadDiscord() {
        // discord app
        loadStyle("/assets/0.28962297433e08e6d793.css");
        loadScript("/assets/062ede8de2fa6d8de1f6.js");
        loadScript("/assets/f908e0dccf3f5c6e9128.js", () => {
            let support = "http://discrod.bin.moe/invite/anime";
            PATCHER_ENV.replaceModule(7004, '---changelog---\ndate: "2018-10-16"\nrevision: 1\nyoutube_video_id: \"GWES8m2OCcw\"\nlocale: "en-us"\n---\n welcome to discrod! {fixed margintop}\n=====================\n\n* anime is cool.\n* lmao discord get raped.\n* [Join our cool server](' + support + ')');
            //console.log(PATCHER_ENV.getModule(7004));
        });
    }
    ...
    document.addEventListener("DOMContentLoaded", e => {
        ...
        if (token) {
            log("Injecting new token before Discord steals it...");
            store.removeItem("inject_token");
            store.setItem("token", token);
        }

        loadScript("/discrod/pako.js");
        loadScript("/discrod/webpack.js", loadDiscord);
    });
})();